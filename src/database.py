...
def init_air_db(reset: bool = True) -> None:
    conn = psycopg2.connect(DB_URL)
    cur = conn.cursor()

    try:
        print("Creating Egypt Airways PostgreSQL database...")
        if reset:
            # CASCADE ensures dependencies are dropped cleanly in Postgres
            cur.execute("DROP TABLE IF EXISTS tickets, bookings, flights, loyalty_program, customers, crew, aircraft, airports CASCADE")
        ...
        cur.execute("""
        CREATE TABLE crew (
            crew_id SERIAL PRIMARY KEY, name TEXT NOT NULL, position TEXT NOT NULL,
        ...
        CREATE TABLE customers (
            customer_id SERIAL PRIMARY KEY, first_name TEXT NOT NULL, last_name TEXT NOT NULL,
        ...
        CREATE TABLE loyalty_program (
            loyalty_id SERIAL PRIMARY KEY, customer_id INTEGER NOT NULL, points_balance INTEGER NOT NULL DEFAULT 0,
        ...
        CREATE TABLE flights (
            ...
            departure_city TEXT NOT NULL, arrival_city TEXT NOT NULL, departure_time TIMESTAMP NOT NULL,
            arrival_time TIMESTAMP NOT NULL, aircraft_id TEXT NOT NULL, captain_id INTEGER NOT NULL,
        ...
        CREATE TABLE bookings (
            booking_id SERIAL PRIMARY KEY, customer_id INTEGER NOT NULL, flight_number TEXT NOT NULL,
            booking_date TIMESTAMP NOT NULL, seat_number TEXT NOT NULL, travel_class TEXT CHECK (travel_class IN ('Economy', 'Business', 'First')),
        ...
        CREATE TABLE tickets (
            ticket_id SERIAL PRIMARY KEY, customer_id INTEGER NOT NULL, booking_id INTEGER,
            problem TEXT NOT NULL, status TEXT CHECK (status IN ('Open', 'In Progress', 'Resolved', 'Closed')) NOT NULL,
            created_date TIMESTAMP NOT NULL, last_update TIMESTAMP NOT NULL, FOREIGN KEY (customer_id) REFERENCES customers(customer_id))""")
        ...
        execute_batch(cur, "INSERT INTO airports VALUES (%s, %s, %s, %s, %s)", AIRPORTS)
        execute_batch(cur, "INSERT INTO aircraft VALUES (%s, %s, %s, %s, %s, %s, %s)", AIRCRAFT_MODELS)
        ...
        execute_batch(cur, "INSERT INTO crew (name, position, nationality, hire_date, base_airport) VALUES (%s, %s, %s, %s, %s)", crew_members)
        ...
        execute_batch(cur, "INSERT INTO customers (first_name, last_name, email, phone, passport_number, nationality, date_of_birth, loyalty_tier) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)", customers)
        ...
        execute_batch(cur, "INSERT INTO loyalty_program (customer_id, points_balance, join_date, last_activity) VALUES (%s, %s, %s, %s)", loyalty_rows)

        flight_numbers = [f"MS{i:03d}" for i in range(1, 101)] # Changed LA to MS for Egypt Airway
        cur.execute("SELECT crew_id FROM crew WHERE position = 'Captain'")
        captains = [row[0] for row in cur.fetchall()]
        cur.execute("SELECT crew_id FROM crew WHERE position = 'First Officer'")
        first_officers = [row[0] for row in cur.fetchall()]
        cur.execute("SELECT crew_id FROM crew WHERE position IN ('Flight Attendant','Senior Flight Attendant')")
        attendants = [row[0] for row in cur.fetchall()]
        ...
        execute_batch(cur, "INSERT INTO flights (flight_number, departure_airport, arrival_airport, departure_city, arrival_city, departure_time, arrival_time, aircraft_id, captain_id, first_officer_id, flight_attendant_id, status, economy_price, business_price) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)", flights)
        ...
        for _ in range(500):
            ...
            cur.execute("SELECT economy_price, business_price FROM flights WHERE flight_number = %s", (fn,))
            econ, biz = cur.fetchone()
        ...
        execute_batch(cur, "INSERT INTO bookings (customer_id, flight_number, booking_date, seat_number, travel_class, price_paid, status, special_requests) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)", bookings)
        ...
        cur.execute("SELECT booking_id FROM bookings LIMIT 5")
        some_bids = [row[0] for row in cur.fetchall()]
        for i in range(len(some_custs)):
            created = fake.date_time_between(start_date='-30d', end_date='now', tzinfo=timezone.utc)
            cur.execute("INSERT INTO tickets (customer_id, booking_id, problem, status, created_date, last_update) VALUES (%s, %s, %s, %s, %s, %s)", (some_custs[i], some_bids[i] if i < len(some_bids) else None, problems[i], random.choices(TICKET_STATUSES, weights=[20, 30, 40, 10])[0], _utc_str(created), _utc_str(fake.date_time_between(start_date=created, end_date='now', tzinfo=timezone.utc))))
